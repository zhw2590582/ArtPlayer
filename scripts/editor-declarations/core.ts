...
export function asGlobalDeclaration(code: string, globalName: string) {
  ...
  const defaultDeclaration = defaultName && declarations.get(defaultName)
  assert(defaultName && defaultDeclaration && ts.isClassDeclaration(defaultDeclaration), 'Core editor requires the actual default constructor class')
  ...
  const qualified = (name: string) => factory.createQualifiedName(factory.createIdentifier(definitionName), factory.createIdentifier(name))
  ...
  const namespace = (name: string, nodes: ts.Statement[]) => factory.createModuleDeclaration(
...
export function generateCoreEditorDeclaration() {
  ...
  assert(code, 'Core bundle is empty')
  const declaration = asGlobalDeclaration(code, 'Artplayer')
  for (const compiler of [ts, compat])
    assert.deepEqual(checkCoreEditorDeclaration(declaration, compiler as unknown as typeof ts), [], `Invalid editor declaration: TS ${compiler.version}`)
  return declaration
}

export function checkCoreEditorDeclaration(code: string, compiler: typeof ts = ts) {
