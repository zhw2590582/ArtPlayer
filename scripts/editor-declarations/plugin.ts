...
import { exportedDefinition, parseDeclaration } from './syntax.ts'

// Migrate each package explicitly; unknown imports/exports must not disappear silently.
export function generatePluginEditorDeclaration(code: string, name: string, localTypes: Record<string, string> = {}, dependencies: ts.Statement[] = []) {
  assert(/^[a-z_$][\w$]*$/i.test(name), 'Invalid plugin global')
  let source = parseDeclaration(code)
  const expanded: ts.Statement[] = []
  for (const node of source.statements) {
    ...
    assert(ts.isStringLiteral(node.moduleSpecifier), 'Expected a literal type module')
    ...
    const types = parseDeclaration(dependency, 'dependency.d.ts')
  ...
  for (const node of source.statements) {
    if (ts.isImportDeclaration(node)) {
      assert(ts.isStringLiteral(node.moduleSpecifier), 'Expected a literal import')
      if (dependencies.length && node.moduleSpecifier.text === '@glomex/vast-ima-player') {
        const bindings = node.importClause?.namedBindings
        assert(node.importClause?.isTypeOnly && !node.importClause.name && bindings && ts.isNamedImports(bindings), 'Expected type-only SDK imports')
        assert.deepEqual(bindings.elements.map((item) => {
          assert(!item.propertyName)
          return item.name.text
        }).sort(), ['Player', 'PlayerOptions'])
        continue
      }
    ...
    if (ts.isModuleDeclaration(node)) {
      assert(node.flags & ts.NodeFlags.GlobalAugmentation, 'Only explicit global augmentations are supported')
      assert(node.body && ts.isModuleBlock(node.body) && node.body.statements.every(item => ts.isInterfaceDeclaration(item) && item.name.text === 'Window'), 'Only Window augmentations are supported')
      globals.push(node)
      continue
    }
    ...
    if (ts.isVariableStatement(node)) {
      ...
      assert(values.length === 1 && value && ts.isIdentifier(value.name) && value.name.text === name && value.type && ts.isFunctionTypeNode(value.type), 'Unexpected plugin callable variable')
  ...
  const namespace = (identifier: string, statements: ts.Statement[]) => factory.createModuleDeclaration([factory.createModifier(ts.SyntaxKind.DeclareKeyword)], factory.createIdentifier(identifier), factory.createModuleBlock(statements), ts.NodeFlags.Namespace)
  const statements = [
    ...globals,
  ...
  const dependencySource = dependencies[0]?.getSourceFile()
  assert(dependencies.every(node => node.getSourceFile() === dependencySource), 'SDK definitions must share one source file')
  const dependencyNamespace = dependencySource
...
function generateCommonJSPluginEditor(source: ts.SourceFile, name: string) {
