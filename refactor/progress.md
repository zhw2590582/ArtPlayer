...
## 2026-09-16：DOC-HANDOFF-01 停止自主重构，转交用户自测

用户要求尽快停止长任务，非必要问题以后处理，不马上发布 npm。已中断会话 34354，
退出码 1；原长测进程和 8084 监听已停止，未完成断言不算通过。
[交接与延期清单](self-test-handoff.md)覆盖以下所有历史“继续”安排。
主计划原 24 doing / 36 todo 共 60 项改为 deferred，并保留原状态、证据与门槛；
新增交接任务后为 227 done / 60 deferred。已知风险不关闭，无新源码修改。
按用户补充要求生成[遗留问题完整清单](remaining-issues.md)，保留主台账全部 77 条开放
记录、5 条范围接受差异、独立分支额外问题及 60 项延期任务，逐项链接证据与关闭条件。
独立分支 1af2c1347 保存排队帧修复，尚未合入；原生专项和默认模式 AV 问题延期。
本轮仅文档/计划校验与提交审计，不再修复、重跑矩阵、打包、复盘、推送或发布。

## PKG-MB-09 长测 AV 阈值越界（运行未终结）

首批两个Chromium组合约1767秒时累计最大帧/音频时钟差达到549ms/570ms，超过
250ms门槛。仍visible、未暂停、无媒体错误，绘制/时钟继续推进；不能据此归因于核心
或系统。新增MB-SYNC-01并保留[快照与诊断边界](changes/2026-09-16-PKG-MB-09-av-breach.md)。
被测输入不变，原进程继续，等待阶段统计/清理和终结报告；不称为通过，不重启到绿。

## DOC-03 校正根 AI 接续指引

根AGENTS仍引用MediaBunny五个旧JS路径及过期源码风格，现已核对并改为实际TS入口，
补充包内架构地图、已批准的根/runtime类型边界和长测独占8084的接续规则。
[记录](changes/2026-09-16-DOC-03-ai-entrypoints.md)。仅文档修正；没有开始正式复盘，
没有改变运行中的长测输入。任务计数不变。
...
匹配，299归档成员与构建/安装副本一致。仅Thumbnail的ARCHITECTURE.md内容改变，
所有运行时/声明字节不变。21库build绑定重新有效，见
[记录](changes/2026-09-16-REL-02-site-inputs-refresh.md)与
[登记检查](baselines/site-inputs-candidate-refresh-check.json)。没有把旧浏览器/设备
报告重新标为新候选证据。REL-02继续doing，225/23/37不变，站点候选仍独立待办。
继续剩余包验收与发布准备，不进入用户保留的正式复盘，不推送/发布。

## PKG-TOOL-THUMB-05 三代核心组合检查点

新增关联核心 3.5.31 完整归档校验和真实文件抽帧/核心预览组合，见
[记录](changes/2026-09-16-PKG-TOOL-THUMB-05-core-combinations.md)。源码36项、
已安装main18项通过，其中分别24/12项实际抽帧，12/6项为Windows WebKit能力控制。
错误通知等待修正后源码WebKit12项跟进通过。旧工具短延时黑格和旧核心取格缺陷
明确记录；重构工具在两种策略、三代核心下通过实际截图与URL释放检查。
05保持doing，Safari/设备及完整历史工具分发缺口保留。225 done /23 doing /37 todo。
共享输入对应候选仍待REL-02刷新；不进入用户保留的正式复盘，不推送或发布。

## SITE-05 本地链接、编辑器与远端缺口

新增check:site-links并接入CI：67生成页面/5766本地引用/942搜索结果全部有效。
真实编辑器与文档三浏览器72项、构建11项、CI回归80项均通过，声明/工具链/类型检查
通过，见[记录](changes/2026-09-16-SITE-05-links-editor.md)。外部链接经系统代理HEAD
复测81个：42个200、39个404；404全部是新增指南尚未进入GitHub master的编辑源文件。
不擅自推送/合并，SITE-05保持doing，225 done /22 doing /38 todo。后续远端源码可用
时复验链接；SITE-06还保留字体许可门槛。共享候选输入需REL-02刷新，正式复盘仍等待用户。

## SITE-05 搜索交互修复

修复 Enter 默认表单提交造成的整页重载、有内容时 Escape 不关闭、查询变化后选择
残留和聚焦时序；保留第三方模板、CSS、索引与结果路径。FlexSearch dev peer 对齐
0.7.43，锁文件没有其他依赖漂移。三引擎双语搜索/文档导航12项与8项构建测试通过，
严格工具链、类型、lint、inventory检查通过；见[记录](changes/2026-09-16-SITE-05-search.md)。
SITE-05已开始但全量链接/搜索锚点/编辑器验收未完成，225 done /22 doing /38 todo。
字体许可仍是SITE-06最终交付直接门槛；共享输入变化后的21库候选需重新绑定证据。
不进入用户保留的正式复盘。

## SITE-04 文档内容核对完成

最后63条显示/尺寸/截图/预览声明已核对，核心963/963的唯一映射、源码/指南哈希
和HTML锚点全部通过检查。20生态包40份指南与已记录的语义核对版本一致，源码、
类型和维护文档无后续变化；详见[记录](changes/2026-09-16-SITE-04-complete.md)与
[证据](baselines/site04-display.json)。122项回归、93项source浏览器用例、10份TS、
六组页面/36次转发通过。原生PiP仅Chromium/Firefox成功，WebKit为明确不支持；
受控布局、真实解码和设备缺口分开记录。两份state示例已纠正，生产API不变。
SITE-04完成，225 done /21 doing /39 todo；21库build有效，其余发布门槛保留。
接续SITE-05/EX-03等实施验收，不进入用户保留的正式复盘。

## SITE-04 播放、切源与画质契约

补齐播放返回/拒绝、数值与只写属性、切源取消、位置恢复和画质选择说明，纠正
switchUrl resolve即代表可播放的旧结论。新增71条映射，累计900/963，剩余63条。
85项回归、24项真实浏览器旧新对照、8份严格TS与6组页面/36次转发通过，见
[记录](changes/2026-09-16-SITE-04-playback.md)与[证据](baselines/site04-playback.json)。
94个旧示例及运行时不变；媒体运行属于source范围，不冒充安装包或真机验收。
21库build有效，SITE-04仍doing，224/22/39不变；接续显示等属性，正式复盘暂缓。

## SITE-04 实例生命周期与媒体能力

补齐实例身份、状态字段、服务归属、reset/destroy 与原生/代理媒体能力说明，
对照发布包纠正 env/build 文档；新增61条映射，累计829/963，134条待核对。
146项测试、10份严格TS、12组页面/42次转发和三浏览器各38项断言通过，见
[记录](changes/2026-09-16-SITE-04-lifecycle.md)与[证据](baselines/site04-lifecycle.json)。
WebKit首次页面加载超时保留报告，同输入/断言/超时的专项重跑通过，未掩盖首次
失败。118个原有示例与运行时不变，21库build有效。SITE-04仍doing，224/22/39
不变；继续播放和显示接口，不进入正式复盘。

## SITE-04 CSS 变量与主题接口

补齐43个CSS变量的默认值、实际用途与模式覆盖，明确cssVar/theme读写及历史类型
边界；新增48条映射，累计768/963，195条待核对。47项测试、两份严格TS、六组
页面/12次转发与三浏览器各83项断言通过（含51项默认值比对），见
[记录](changes/2026-09-16-SITE-04-css.md)与[证据](baselines/site04-css.json)。
94个既有示例及运行时不变，21库build有效。SITE-04继续doing，224/22/39不变；
继续播放与实例接口，不进入正式复盘。

## SITE-04 构造配置与初始化边界

新增双语默认值、合并归属、覆盖顺序、平台开关和分阶段回调说明，纠正缩略图/
字幕默认值；新增93条映射，累计720/963，243条待核对。135项测试、两份严格TS、
六组页面/24次转发及三浏览器各34项断言通过，见
[记录](changes/2026-09-16-SITE-04-options.md)与[证据](baselines/site04-options.json)。
运行时与106个既有示例不变，21库build有效。SITE-04继续doing，224/22/39不变；
接续播放属性、CSS变量与实例接口，仍不进入正式复盘。

## SITE-04 事件与同步派发契约

新增双语事件语义说明与120条声明映射，累计627/963，336条待核对。131项测试、
两份严格TS、六组页面/24次转发和三浏览器各30项事件断言通过，见
[记录](changes/2026-09-16-SITE-04-events.md)与[证据](baselines/site04-events.json)。
首轮seek失败由旧8082服务缺少Range支持复现；原生video对照及当前服务8083验证
确认，保留失败报告，未修改播放器或放宽断言。21库build仍有效。SITE-04继续doing，
224/22/39不变；继续构造选项与播放等剩余内容，正式复盘仍由用户指导。

## SITE-04 静态配置与全局常量

补齐34个全局字段的共享范围、默认值与读取时机，修正config/resize/scroll/恢复
阈值/销毁说明。新增44条映射，累计507/963，456条待核对。117项测试、四份严格
TS、12组页面/18次转发和三浏览器各52项断言通过（含34项默认值比对），见
[记录](changes/2026-09-16-SITE-04-globals.md)与[证据](baselines/site04-globals.json)。
源码未改，21库build有效；SITE-04继续doing，224/22/39不变。不进入正式复盘。

## SITE-04 公共工具函数语义映射

补齐50个工具成员的双语行为与资源归属指南，新增105条根/runtime/结构输入/入口
映射，累计463/963，500条仍待核对。57项测试、两份严格TS、六组页面/72次代码
转发和三浏览器各41项工具断言通过，见[记录](changes/2026-09-16-SITE-04-utils.md)
与[证据](baselines/site04-utils.json)。核心源码不变，21库build仍有效。SITE-04保持
doing，224/22/39不变；继续构造配置、事件和播放等文档，不进入正式复盘。

## SITE-04 字幕语义映射

补齐双语字幕加载、渲染、配置归属与清理指南，新增27条声明映射，累计358/963，
605条仍待核对。34项测试、六份TS、六组页面/108次代码转发，以及三浏览器各22项
原生track/渲染/取消断言通过。见[记录](changes/2026-09-16-SITE-04-subtitle.md)和
[证据](baselines/site04-subtitle.json)。只刷新字幕章节相关指南哈希，其余章节与
源码已核对不变。21库build有效；SITE-04继续doing，224/22/39不变，不进入正式复盘。

## SITE-04 插件注册语义映射

补齐双语插件编写指南，明确同步/异步注册、ready独立、命名/返回值、工厂构造阶段、
销毁清理与根/runtime类型边界。新增7条声明映射，累计331/963，632条仍待核对。
27项测试、两份严格TS、六组页面/18次代码转发及三浏览器各23项注册断言通过，见
[记录](changes/2026-09-16-SITE-04-plugin-registry.md)和[证据](baselines/site04-plugin-registry.json)。
核心源码未改，21库build证据仍有效。SITE-04保持doing；224/22/39不变，继续实施，
不进入正式复盘。下一批为字幕和剩余核心成员。
...
累计324/963，639条仍待核对。六组页面、54次代码转发、三浏览器各40项DOM断言及
完整示例的延迟更新通过。新表格触发翻译分段器结构错误，已修复并增加两项回归；
最终42项测试、严格类型及全量lint通过。见
[记录](changes/2026-09-16-SITE-04-setting.md)及[证据](baselines/site04-setting.json)。
共享脚本/测试变更使21库旧build输入指纹过期，REL-02需重新准备验证候选，不能重标旧报告。
SITE-04保持doing，224/22/39不变；继续实施，不进入正式复盘。

## SITE-04 核心公共组件语义映射

核对layers/controls/contextmenu的公共接口、配置修改、浅合并替换、回调/清理、
返回值与选择器异步归属，补齐六份双语指南；73条新声明映射使累计达到245/963，
718条仍待核对。六份严格TS、18组页面、72次代码转发、三浏览器各38项DOM断言及
35项现有测试通过。见[记录](changes/2026-09-16-SITE-04-components.md)及
[证据](baselines/site04-components.json)。文档初次构建的泛型文本HTML解析失败已修复并保留日志。
21库build证据仍有效；SITE-04保持doing，224/22/39不变，不进入正式复盘。

## SITE-04 核心模板和图标语义映射

补齐高级属性和静态属性的四份双语指南，明确节点/查询、SSR、静态HTML、图标节点
复用及历史类型边界。新增91条声明映射，累计172/963，791条仍待核对。四份严格TS、
12组页面、18次代码转发、三浏览器各63项DOM断言及31项现有测试通过；首次LLM
生成滞后的失败日志保留，重新生成后同组检查通过。见
[记录](changes/2026-09-16-SITE-04-template-icons.md)及[证据](baselines/site04-template-icons.json)。
21库build证据仍有效；SITE-04保持doing，224/22/39不变，继续其他核心组，不进入正式复盘。
...
1469个相关输入保持一致，10个归档/安装成员匹配，只有ARCHITECTURE与旧候选不同。
重新登记后21库build证据均有效。见[准备与验证](changes/2026-09-16-REL-02-dash-refresh.md)。
不重标旧浏览器报告，不关闭SDK/媒体/真机或正式复盘门槛；REL-02保持doing。

## SITE-04 早期五个插件指南的当前语义核对

完成 Danmuku/HLS/DASH/Audio/VTT 十份双语指南的源码/声明对照：修正文本空格、
异步刷新措辞，补齐内部返回对象、数据修改、节点复用、图标/滑块边界及端点行为，
恢复HLS/DASH按钮文字。10份严格TS、30组真实浏览器页面与78次代码转发、17项工具
检查通过。见[记录](changes/2026-09-16-SITE-04-existing-guides.md)和
[证据](baselines/site04-existing-guides.json)。核心963条语义映射仍待完成，SITE-04
保持doing、224/22/39不变；不进入正式复盘，不将页面转发计作媒体/真机验收。
DASH包内维护说明同步修正，旧候选指纹已过期，需紧接REL-02重建；其余20库仍有效。
...
快照文件和299个归档成员，台账接受21份build证据且无过期指纹。20包隔离消费
及Thumbnail的12组类型检查通过；本批安装产物的弹幕/Audio聚焦浏览器36项通过。
核心strict预检仍以28项缺口退出1，未把构建通过当作可发布。见[记录](changes/2026-09-16-REL-02-candidate-registration.md)
与[候选证据](baselines/major-candidate-preparation.json)。文档站候选、完整组合、
实际候选回退、真机和其他门槛仍未完成；REL-02保持doing，224/21/40不变。
下一步使用这些准确归档推进剩余验收；输入变化须重建，复盘继续等待用户指导。
...
来源按 kind 判别，源码也执行插件直接销毁后再销毁宿主。新增真实 CLI 收集正反例，
20项 Node 检查通过。最终三引擎18项：15通过、3个旧版WebKit像素失败、0跳过/重试；
候选9项均通过，并在宿主仍存活时证明worker仅终止一次、画布清零。

完整源码只重新收集：174文件/1667项，并非执行通过；旧安装映射只收集6项，
不计为新包播放验收。见[记录](changes/2026-09-15-CI-JASSUB-SOURCE-01-native-inputs.md)、[证据](baselines/jassub-source-selection-validation.json)。
生产代码、类型、分发与依赖未变，CI-01和JASSUB完整兼容/设备门槛继续开放。
新增任务独立完成后为217 done、21 doing、43 todo，共281。下一步检查其他原生
用例是否存在相同候选输入缺口；不重复无变化的全量运行，不推送或发布。


## CI-01 完整 Windows WebKit 源码回归

固定ae56937d3、Node24.21.0/Yarn1.22.22，174文件/1664项完整运行：
1515通过、11失败、138能力跳过、0重试，1816.373秒，实际退出1。
8项音频用例初始双时钟已通过，但均未到达规定的可信waiting；没有完成恢复断言。
3项JASSUB默认加载的是npm1.1.0，资源成功加载、媒体时钟推进，但零帧计数使
帧回调和字幕需求未交付。保留原失败，候选已修路径和未验证范围不混写。

58项ASR WebAudio、48项DASH MSE及32项HLS MSE跳过原因逐文件保存；
通过数也包含历史/受控/不支持分支。Monaco修复在本轮全量中通过。
见[记录](changes/2026-09-15-CI-01-source-webkit.md)和
[证据](baselines/ci-source-webkit-validation.json)。CI索引汇总三引擎各自HEAD、
范围与结果；没有拼成同一候选绿色矩阵，未关闭先前间歇风险或物理设备缺口。

CI-01继续doing，216 done、21 doing、43 todo不变。只更新证据和维护文档，
不改测试/生产/工作流，不推送或发布。下一步核对source中仍只默认选择历史包
的原生用例，补足候选输入，同时保留历史控制及原始失败。
...
保留BASE-04快照，增加显式pages增量、任务/路由/提交依据校验和有效负例；实际Git
逐一核实九个引入提交。未来同次提交的新页面可登记修改前HEAD，避免自引用哈希。

最终完整ci:check退出0，476.52秒：运行时2937、工程170、历史基线619通过；
416生产TS、旧/新类型模式、React/Vue、lint及串联产物检查通过。三个定向台账
测试通过。见[记录](changes/2026-09-15-SITE-ROUTES-01-html-inventory.md)和[证据](baselines/site-routes-validation.json)。

这是现有安装的本地检查，不是完整浏览器/coverage/构建/远端CI或发布验收。
分支影响分析涉及22包，650未分类路径仍保守审查，通用安装消费者19包缺口保留。
CI-01仍doing；213 done、21 doing、43 todo，共277，无推送/部署/发布。
...
失败/取消、没有更新接口及历史类型迁移。两页Run Code与原demo逐字一致。

最终12项浏览器通过：新旧核心×三引擎的实际裁剪像素/边界/空档/销毁6项，
双语导航6项。首轮缺事件参数及WebKit绕过媒体拦截的6项失败保留；修正测试，
所有引擎使用原demo视频，没有放宽断言。17项文档工具、7项站点测试、两份严格
TS、链接/构建/lint通过；内置浏览器仅确认原demo加载。见[记录](changes/2026-09-15-SITE-VTT-01-guides.md)和[证据](baselines/vtt-docs-validation.json)。

211 done、21 doing、43 todo，共275；站点36 Markdown/45 HTML，中英各18页。
VTT05/06与SITE-04门槛保留，无生产源码/API/类型/依赖/版本修改，无推送发布。
...
音轨lang为null时抛错。修正为局部SDK所有权与单个最终监听器、固定能力分支和
字符串标签回退，三项回归通过；SDK默认设置和插件API不变。

新增双语指南和侧栏，解释两代SDK选择键、回调、同步update、事件刷新和所有权。
两页完整示例与原demo逐字一致。真实SDK播放/换源清理8项、不支持路径4项、
导航6项一次全部通过；Node/文档工具20、站点测试7、两份严格TS片段和lint通过。
8082内置浏览器观察到新代码、视频12:14和画质/音轨菜单，只计页面加载。
见[记录](changes/2026-09-15-SITE-DASH-01-guides.md)与[证据](baselines/dash-docs-validation.json)。

SITE-DASH-01完成、DASH-EXAMPLE-01关闭；原完整SDK/设备/发布任务保持未完成。
站点32 Markdown/41 HTML，中英各16页；209 done、21 doing、43 todo，总273。
没有插件生产源码/公开类型/版本/锁文件修改；无推送、部署或发布。

## SITE-HLS-01 双语指南与示例修复

原HLS示例已复现换源累积destroy监听器及原生回退误装SDK插件，回归2失败/1通过。
现在每个实例销毁一次、每个播放器只有一个清理监听器，原生分支不安装HLS控制插件。
新增中英文指南与侧栏，配置、同步刷新、SDK所有权、类型和未发布边界明确；
两个完整示例与原demo逐字一致。站点增为30 Markdown/39 HTML，中英各15页。

最终浏览器30通过：16真实SDK播放/换源清理、8 WebKit不支持路径、6双语导航。
首轮8个WebKit失败来自测试误读notice.show getter，修正后重跑；保留原报告。
Node/文档工具20通过、站点构建测试7通过、两个原样TS片段strict NodeNext零诊断，
lint与生成检查通过。内置浏览器8082观察到新代码、视频元数据和画质/音轨菜单，
该观察只算页面加载。详情见[记录](changes/2026-09-15-SITE-HLS-01-guides.md)和[证据](baselines/hls-docs-validation.json)。

SITE-HLS-01完成，HLS-EXAMPLE-01关闭；SDK原生崩溃/停滞和设备任务保持未完成。
208 done、21 doing、43 todo，总272。无插件生产源码/公开类型/版本/锁文件变更，
新增测试已进入既有CI测试脚本；没有推送、部署或发布。
...
约1秒、未暂停且缓冲区为空。独立trace中最终切组后没有新的视频片段请求，
42条媒体请求均200；不是仅有菜单或网络错误的推断。保留三通过一失败的原轮。

增加音轨选择/完成/同调用切组及整段缓冲对照，双向档位切换要求真实时钟和帧
增长；19次有效诊断通过不能解释失败。修正两处诊断采集问题，失败报告独立保留。
最终分组矩阵8通过、4 Windows WebKit MSE跳过；lint、来源单测和工具链通过。
见[记录](changes/2026-09-15-PKG-HLS-SDK-01-switch-boundary.md)及
[证据](baselines/hls-switch-boundary-validation.json)。任务和两项风险继续开放，
总数203 done、21 doing、43 todo不变。后续围绕控制器/分轨缓冲状态定位，
不更改消费者SDK参数或以反复通过宣布修复；未推送、部署或发布。
...
捕获图继续运行。新增对 `video:error` 的停止/销毁解绑，复用现有取消、资源归属
与恢复逻辑。五项单测旧红新绿，错误中加载/延迟关闭、旧回调和恢复均覆盖。

234 ASR 专项、main/legacy 各43生命周期及6清单测试通过。源码/两份实际安装
产物共80浏览器通过、40 Windows WebKit WebAudio能力跳过；正常音量/静音、
无音轨、跨域及三种核心的错误恢复纳入回归。三种正式构建字节与独立安装一致。
初版 Firefox 等待首个静音块的测试死锁已修正并保留失败记录。

见[修复记录](changes/2026-09-15-PKG-ASR-10-media-error.md)和
[证据](baselines/asr-media-error-validation.json)。任务10完成，任务05继续doing；
203 done、21 doing、43 todo，共267项。类型/依赖/锁未改，远端CI/设备/发布复盘
仍待完成；未推送、部署或发布。
...
跨归档完整性反例纳入，单元10/10、严格类型/lint/生成检查通过。见
[记录](changes/2026-09-15-SITE-07-console-commonjs.md)。剩余27模块与Parcel及完整
许可继续核查；SITE-07 doing，仍199/265。没有运行时、锁文件、推送或发布变化。

## SITE-07 console-feed 32 模块精确重建

官方 npm 3.2.2 的 32 个模块使用历史压缩配置全部重建为旧 bundle 原字节；首次
下载和离线复现均通过，新增完整依赖遍历及篡改反例，单元 9/9。来源与许可原文
已保存，见[记录](changes/2026-09-15-SITE-07-console-feed-source.md)。其余 68 个
第三方模块/Parcel 和完整许可仍开放；SITE-07 doing，仍为199/265，无运行时变更。

## SITE-CONSOLE-01 自有 TS 控制台与三项修复

完成入口/视图/订阅/错误/样式/类型拆分，保留原 React17 与 100 个第三方模块。
多面板 hook、卸载滚动、错误消息三项已复现风险已修复，编辑器 pagehide 清理一并
接入。最终组合 63/63、共享解析 3/3，单元 6+10 通过；详见
[记录](changes/2026-09-15-SITE-CONSOLE-01-typescript.md)。199/265 项完成。
SITE-07 / VENDOR-08 的完整源码及许可仍开放；下一步继续该边界，未推送或发布。

## SITE-07 控制台基线与三项缺陷

冻结 102 模块和真实全局/返回组件/DOM/日志/滚动边界。三引擎最终27/27为18项共同
行为和9项旧缺陷复现，不能读作已修复；生产脚本不变。见
[记录](changes/2026-09-15-SITE-07-console-baseline.md)。新增 SITE-CONSOLE-01 与三项
开放风险，下一步实施自有 TS 模块和生命周期/错误修复；198/265 项完成。

## ENG-LINT-02 全仓 lint 零错误、零警告

修正 VAST 顶层清单顺序，从生成源去掉无效 lint 禁用项并重新生成。元数据值与
嵌套条件顺序不变，声明只改注释；24 个输出经过两版 TS 校验及只读复核，根 lint
零错误/警告。见[记录](changes/2026-09-15-ENG-LINT-02-clean-checks.md)。
本修复单独完成：198/264 项；SITE-07 继续 console.js 来源与剩余站点资产工作。
...
新增完整通知关联校验；16/16 单元与工程测试、三引擎真实编辑器/移动播放/HTTP 原文
6/6 通过。见[记录](changes/2026-09-15-SITE-07-codicons.md)。197/263 完成，SITE-07
仍 doing；其余组件通知、字体媒体与远端/真机验收继续跟踪。下一步补 vConsole 许可。

## CI-01 VAST 完整安装包与真实 IMA

二十包安装准备通过。VAST 完整 glomex 加载边界在源码、安装范围各 36/36；
真实 IMA 3.789.0 专项 39/39（30 安装候选、9 历史对照），三个引擎均无失败/
跳过/重试，覆盖播放、303 恢复、重建、活跃广告销毁与真实跳过按钮。单元/工程
94/94，见[记录](changes/2026-09-15-CI-01-vast-installed.md)。完整 installed
清单现为 73 文件/2,442 项，本轮未全跑。197/263 完成；CI-01/VAST-05 仍 doing，
以前的间歇失败、物理设备、其余分发和远端 CI 门槛保留。

## CI-01 Iframe 安装包与真实跨窗口组合

统一清单现含十九包，保留 Iframe 真实旧 npm 包与冻结 tool 工作区的不同身份。
五文件三引擎 426/426 全部通过，零跳过/失败/重试；覆盖历史协议、窗口/文档
边界、导航、新旧核心/父子工具和实际编辑器。源码定向 3/3、单元及工程 113/113。
见[记录](changes/2026-09-15-CI-01-iframe-installed.md)。完整安装 collection
为 72 文件/2,406 项，并未全跑；197/263 完成。CI-01/IFRAME-05 继续 doing，
其余生态分发、特殊 BFCache、设备和远端矩阵门槛保留。

## CI-01 浏览器引擎与消费者作业拆分

播放检查改为 3 个 OS × 3 个引擎；Node/React/Vue/iframe history/性能每个 OS
独立运行一次。CI result 要求四组作业全部成功，失败上传与 source/installed
双范围保留。全量清单按测试 ID 核对：源码 4,641、安装 1,980 项，拆分无遗漏
或重复；定向真实运行 93 通过、12 个 WebKit ASR 能力跳过，工程测试 77/77。
见[记录](changes/2026-09-15-CI-01-browser-matrix.md)。197/263 完成，CI-01
仍 doing；远端耗时、缓存、调度与完整矩阵尚未验证，不关闭既有失败或设备缺口。

## CI-01 弹幕 / Mask 安装与原生负载矩阵

十八包打包/隔离安装通过；十三文件、三引擎 399/399 断言通过，22.5 分钟，
保留历史对照、真实模型、#958、两档三轮压力、全屏和 PiP 能力差异。
源码定向 3/3、单元 323/323、工程 49/49 通过，见
[记录](changes/2026-09-15-CI-01-danmuku-installed.md)。完整清单为
67 文件/1980 项，本轮未全跑；197/263 完成。后续补 VAST/工具/站点相应
分发覆盖，并按实测耗时拆分 CI 浏览器任务；设备/长期运行和既有失败不豁免。

## CI-01 JASSUB 主包及安装 Worker/WASM 验证

十六包打包/安装通过，JASSUB 五文件实跑 45 项：41 通过、4 失败，实际退出 1。
三项是旧版 WebKit 渲染对照；另一项是已有 Firefox 离屏读回停顿，风险保持开放。
候选 26/27，通过的九项原生字幕三核心/三引擎组合与资源来源均有证据，
字体作为独立资源记录。源码定向 3/3、候选单元 84/84、工程 54/54 通过，
见[记录](changes/2026-09-15-CI-01-jassub-installed.md)。完整安装清单为
54 文件/1581 项，本轮未全跑。197/263 完成；继续弹幕及其 Mask 资源映射，
不关闭 Firefox、设备、许可或全量 CI 门槛。

## CI-01 MediaBunny 接入实际安装矩阵

十五包打包/隔离安装通过，新增 MediaBunny 十文件实跑 228/228 断言通过，
包括原生解码/HLS/音频、PiP 组合与受控/能力对照；WebKit 缺能力的分支不算
播放验收。源码定向 3/3、相关单元 257/257、工程 48/48 通过，见
[记录](changes/2026-09-15-CI-01-mediabunny-installed.md)。完整清单扩展到
49 文件/1536 项，本轮未全跑。197/263 完成，CI-01 与设备/发布门槛仍开放；
下一步接续 JASSUB/弹幕等剩余包，并核对独立 worker/模型资源来源。

## CI-01 统一安装包清单并接入 ASR / Chromecast

新增 --browser 从同一清单准备十四包，实际安装核验通过；消除 CI 手写列表并
保留默认/显式子集方式。新增七文件三引擎 171 项：129 通过、42 WebAudio 能力
跳过；Cast 仅受控 SDK，设备门槛不变。源码定向 3/3、单元 357/357、工程 48/48
通过，见[记录](changes/2026-09-15-CI-01-asr-cast-installed.md)。完整清单为
39 文件/1308 项，本轮未全跑。197/263 完成，CI-01 继续 doing；下一步处理其余
包的完整产物/资源映射，VAST 的依赖替换测试不能冒充已安装完整 bundle。

## CI-01 自适应控制和自动缩略图接入安装矩阵

十二包实际安装通过，清单扩展到 32 文件/1137 项。新增七文件三引擎实跑
375 项：293 通过、2 裸 dash.js 4.5.2 seek 失败、80 Windows WebKit MSE 能力
跳过。Auto Thumbnail 首帧限制仍保留，未把能力诊断计为通过。源码定向 3/3、
相关单元 278/278、工程 47/47 通过；见[记录](changes/2026-09-15-CI-01-adaptive-installed.md)。
CI-01 继续 doing，197/263 完成；后续补齐其余安装覆盖并审查全矩阵耗时与失败归类。

## PKG-HLS-PACK-01 修正实现配置进入 npm 包

十二包安装准备发现 HLS 的内部 tsconfig 未排除。已修正 .npmignore，实际旧包
拒绝/新包通过，六个公开 JS/声明文件与 package.json 字节不变；中间换行拼接
错误也被校验拦截并保留归档。见[记录](changes/2026-09-15-PKG-HLS-PACK-01-config.md)。
此任务独立完成，263 项现为 197 done、22 doing、44 todo；CI-01 接续安装与浏览器。

## CI-01 音频、缩略图和多字幕接入安装矩阵

九包安装与完整文件核验通过，统一安装清单扩展到 25 文件/762 项。本轮新增
十二文件三引擎实跑 336 项：327 通过、9 失败。八项 Windows WebKit 音频断流
保持 AUDIO-BUFFER-01；ASS 的过早单元素断言已修正，随后全部 15 组合通过，
原失败报告保留。源码实际媒体 3/3、相关单元 504/504、工程 46/46 通过。
见[记录](changes/2026-09-15-CI-01-subtitles-installed.md)。CI-01 继续 doing，
完成数仍 196/262；下一步接续剩余包安装覆盖和真实环境门槛，未声称全量 CI 通过。

## PKG-CHAPTER-05 原生时钟对照检查点

默认 trace 与关闭 trace 的两组固定 WebKit 安装组合均为 11/12 通过，保留失败。
被监测媒体属性访问均很短，页面却出现约五秒空档。不加载 ArtPlayer 的原生 video
在一次位置纠正后 6/6 恢复，其中两次也有类似空档；延后纠正仍出现 5104 ms 空档，
因此未改核心或扩大超时。新增显式探针、独立原生入口和[复跑记录](changes/2026-09-15-PKG-CHAPTER-05-native-timing.md)。
具体底层原因仍未确认，05/CHAPTER-TIMING-01 保持开放；任务完成数仍为 196/262。

## CI-01 Ads 接入安装矩阵，完整回归保留章节失败

六包统一 installed 范围实测 426 项：425 通过、1 项 WebKit 章节切画质等待失败；
新增 Ads 171 项全过，108 项确认加载已验证候选 tarball。源码模式定向真实播放
1/1、Node 工程 46/46、类型/CI 静态检查通过。失败 trace 的媒体状态读取耗时
9.94 秒，原因未确认，CHAPTER-TIMING-01 保持 open；没有扩大超时或重跑覆盖。
详见[记录](changes/2026-09-15-CI-01-ads-installed.md)。CI-01 继续 doing，
完成数保持 196/262；下一步调查章节读取延迟并继续其余包安装覆盖。

## MOD-01 Bun 隔离安装评估完成，保留 Yarn

固定 Bun 1.4.2 与 Yarn 1.22.22 在仓库外同提交安装。Bun 初次冻结迁移失败，
普通迁移与随后冻结复装通过；传递依赖和安装脚本策略仍与 Yarn 不同，因此不采用。
两边 2,906 项单元测试、415 个 TS 源文件/类型消费者、完整构建均通过。库输出
与声明一致，文档站存在精确字节差异；不把测试通过当成安装等价。新增可复跑脚本、
审计负例与[记录](changes/2026-09-15-MOD-01-bun-evaluation.md)。默认仍是 Yarn，
根锁不变，未切换运行时/bundler；262 项现为 196 done、22 doing、44 todo。

## REL-04 完整旧站点恢复检查点

修正 Git archive 的本机换行影响：原 ZIP 有 403 处 CRLF 差异，保留失败证据；
命令级固定换行设置后，538 个文件与原始 Git blob 全部一致。隔离站点的脚本/
域名损坏、缺失和遗留文件恢复通过，12 条 HTTP 路径及三引擎六项播放验证通过。
已新增可重跑脚本、预检负例和[维护记录](changes/2026-09-15-REL-04-pages-recovery.md)。
远端 Pages 恢复未执行，Thumbnail 原包/完整替代方案仍缺，REL-04 继续 doing。

## REL-04 更名回退与主线工作流检查点

完成旧 iframe 包→新 tool 包→恢复旧包的实际冻结安装、应用导入/类型/成员核验，
并复现仅映射包名无法保持新构造/legacy 用法。独立 Git 仓库内 JS→TS 冲突的
中止恢复与修复同步均通过；真实 master 无新增待同步提交。22 包恢复清单生成，
20 份完整旧 archive 的双哈希验证通过，新增清单 CI 检查。详见
[记录](changes/2026-09-15-REL-04-workflows-checkpoint.md)。Thumbnail 完整替代
回退产物和站点恢复证据尚缺，REL-04 继续 doing，任务完成数不变。
...
默认恢复回调前Player、id/$container/imaPlayer可写数据字段及SDK原默认设置；
显式模式保留工作区四事件显隐、活动请求抑制、可变设置和资源实时getter。
两者共享销毁终止、失败回滚、旧会话隔离和显式重建修复；模块地图/迁移JS示例同步。

当前Node82通过，三浏览器162通过（99候选、63历史），工程脚本12通过；严格
TS5.9.3、lint和工具链检查通过。首轮浏览器153通过9失败保留：旧断言误用
工作区选择器/显隐语义，现按历史责任分模式并补充回调起始状态断言，未放宽清理。
实际核心为npm5.1.7/npm5.4.0/候选5.4.1，纠正旧维护说明误称published5.4.1。
正常构建三格式及docs副本一致，实际main+ESM/legacy+ESM各4项导出检查通过。

详见[变更](changes/2026-09-14-PKG-VAST-03-compatibility.md)、
[指纹和结果](baselines/vast-compatibility-validation.json)及
[接受的决策](vast-compatibility-decision.md)。本项完成，262项现194 done、
20 doing、48 todo；初始化风险按用户决定限定接受。SDK仍受控，实际IMA恢复、
公开声明（包括第二参数）、安装分发分别留在05/04/06；未用VPN例外，未push/发布。
本任务独立本地提交，提交后先核验审计；下一步处理VAST公开类型04。

## PKG-DANMUKU-MASK-LOAD-01 真实模型负载漏显与积压修复

连续播放采样补回主线程卡顿跨过的既有wait行，公开readys窗口不变；允许重叠
模式直接使用原轨道计算，默认防重叠仍走Worker。采样/纯几何/消息协议拆分，
保留串行回调、取消、显示寿命；document迁移及destroy释放visibility监听器。
没有更换Mask模型/SDK、缩小视频、降低20条每秒负载或扩大原等待预算。

模型源码18通过（9候选完整/9旧观察），main/legacy各39通过（9模型、24寿命、
6时钟）：27个候选模型用例均120行唯一显示/回收，pending/reserved为空。
寿命测试补齐防重叠/允许重叠两条路径，源码60通过；原生PiP矩阵48项中的
32原生播放与16API不可用保持区分。第一次寿命12失败来自旧测试强制Worker
假设，完整报告保留；扩展用例继续验证实际延迟路径，没有降低寿命要求。

Node source/main/legacy各276通过，严格TS5.9.3、4项类型/编辑器测试、实际
隔离安装12组通过。误用TS5.1.6别名及把ESM交给同步VM的失败单独记录，
不归为产品缺陷或ESM行为通过。构建及79个最终打包成员一致性已核验。
见[变更](changes/2026-09-14-PKG-DANMUKU-MASK-LOAD-01-scheduling.md)及
[证据](baselines/danmuku-mask-load-validation.json)。模型帧间隔仍高，未宣称帧率提升。

本缺陷任务及风险关闭；262项：193 done、21 doing、48 todo。弹幕08/09、
Mask05/06及其他包/设备/远端CI/发布复盘继续开放；ESM完整浏览器分发仍属09。
本项按独立本地提交交付并核验审计，未push/发布。下一步继续包组合与分发收尾。
...
12项、storage/auto-playback单元18项及定向lint通过。持久化保留独立对象及交错
设置写入语义，不以缓存改变旧调用；保留WebKit原测试时钟断言失败和最终区间断言。
Chromium真实首帧60次采样补充初始化/销毁归因，不把采样时长当作正式性能结果。
正式安装包三引擎三组配对及54次候选资源窗口见ENG-13证据，体积和间歇计时风险
仍open，继续交REVIEW-01审查。新安装run-jl3UQr通过36项运行时、5组旧类型、8组
精确类型；包内容只有ARCHITECTURE.md变化，运行时/声明与浏览器用包字节一致。
见[验收记录](changes/2026-09-14-MOD-03-performance-acceptance.md)及
[测量证据](baselines/core-performance-validation.json)。255项：185 done、17 doing、
53 todo。本次不关闭插件、真机、远端CI、三轮复盘及发布门槛。下一步继续剩余包的
兼容收尾和工程任务，按任务独立提交；VAST默认行为选择仍待用户回复。

## ENG-13 性能探针观察窗口修复完成

MOD-03新隔离包run-rawoRh通过36项运行时、5组旧类型和8组精确类型消费。正式配对
首轮Firefox有一个candidate资源窗口实际仅349ms，正确触发350ms门槛；保留原失败。
当前页面适配器按同一时钟补足剩余观察时间，保留实际读数，不改冻结夹具、样本循环
或阈值。9项时钟/证据/冻结基线回归、lint和工具链通过；同一安装包重新完成三引擎
全部三组配对，54次candidate资源探针通过，所有旧新观察都达到350ms。
见[修复记录](changes/2026-09-14-ENG-13-observation-window.md)及
[证据](baselines/observation-window-validation.json)。第二轮无计时超阈值，体积仍需
审查；不把首轮波动归因于仅修改观察窗口的修复，也不把review-required改成通过。
255项：184 done、18 doing、53 todo。MOD-03继续持久化和初始化/布局分析；本次没有
改变播放器源码、依赖、公开接口或产物，不代表设备/全插件/远端CI/发布验收完成。

## MOD-03 进度/音量重复ARIA写入优化检查点（仍doing）

核心slider保留每次读取range/文本、即时更新和外部DOM修复，仅在属性值实际改变
时写入。新受控回归旧10通过1失败、新11通过；全项目2859项单元、严格核心TS、
lint、37份声明无漂移和工具链通过。main/legacy三引擎各三组旧新交替配对全部
通过，100次重复事件的ARIA变更由500次降到0；实际播放仍更新进度数值。
main另有54项新旧进度/质量/键盘回归，legacy另有12项键盘/音量回归通过。
计时保留原始值，含变慢组；这些是合成重复事件诊断，不是整体帧率或正式
BASE-06安装包性能准入。现代gzip增加10字节，原体积及间歇计时风险保持open。
见[变更](changes/2026-09-14-MOD-03-slider-updates.md)及
[证据](baselines/slider-updates-validation.json)。正常构建同步三格式/docs并恢复i18n。
254项：183 done、18 doing、53 todo；MOD-03仍需持久化、初始化/布局热点和新的
实际安装包三组完整性能/资源对比。没有调整公开事件频率、默认行为、依赖或版本。
...
对照并记录原生currentTime写入、事件和Promise状态。首轮WebKit27次切源中旧
核心18次有5次立即seek未到达，两个无插件旧核心组也复现；候选9次全部到达。
正式main三引擎81次切源无失位，但旧WebKit仍有6次在seeking中结算；候选27次
均在原生定位结束后结算并兑现首个seek。确认是已有旧核心寻址问题，候选CORE-19
路径已处理，无需在多字幕插件追加重试或改时钟。原失败及立即调用路径完整留证。
84项正式产物矩阵83断言通过，1项测试未等待全屏进入通知即退出而失败；保留
严格true/false顺序断言，补事件谓词后受影响9项三引擎全通过，不声称重跑84项。
脚本lint与CI50项通过。见[归因记录](changes/2026-09-14-PKG-MULTI-SUB-09-switch-order.md)
和[证据](baselines/multiple-subtitles-switch-validation.json)。关闭误归因的SWITCH风险，
无生产、声明、依赖、版本或构建产物修改；249项为177 done、17 doing、55 todo。
任务05仍未完成，下一步处理旧5.1核心的双语渲染适配，再补剩余组合与分发验收。

## CORE-SUBTITLE-OFFSET-01 暂停字幕偏移修复完成

原生video/track探针确认Firefox155暂停时cue时间更新后活动标记陈旧；候选核心
增加独立timing模块，只在暂停且原生cue成员/顺序与当前时间不一致时，将原cue
按原顺序移除并重新加入。保持对象、模式、媒体时间与update/notice/公开偏移事件。
先前mode切换方案会逆转同时间字幕顺序，已弃用并保留失败证据。新增5项风险回归，
核心定向58项、严格核心/消费者TS、37声明零漂移、导入5项及CI50项通过。
扩展源码浏览器153项中150断言通过，旧WebKit两项切源失败保持open，另一个原生
探针时间精度假设已修正。正式main15项（含原生和历史观察）、legacy3项全部通过，
候选覆盖正负/恢复偏移、禁用轨道和恢复播放。lint仅1既有warning。三格式/docs
重建，i18n经正常脚本重建且无差异；main gzip9增加109字节，无依赖、版本或公开
声明改动。见[变更](changes/2026-09-14-CORE-SUBTITLE-OFFSET-01-paused.md)与
[证据](baselines/subtitle-offset-validation.json)。248项：176 done、17 doing、55 todo。
关闭候选偏移缺陷，旧核心的对应失败保留明确观察；这不完成PKG-MULTI-SUB-05。
下一步继续旧核心双语显示适配和WebKit切源定位，再完成剩余包组合/分发门槛。

## PKG-MULTI-SUB-05 新旧核心组合检查点（未完成）

任务08已独立提交675b095b2，提交审计175 done/171专属提交/4初始例外通过。
任务05现进入doing：补取并冻结真实npm核心5.1.2，5.3.1查询404保持缺失；与
现有5.1.7/5.3.0/5.4.0/候选组成矩阵，旧插件1.0.0/1.1.0/1.2.0也实际运行。
57项三引擎检查53通过、4失败、0跳过，其中24项是边界观察，18项确认双语
缺失而非兼容通过。旧5.1.2/5.1.7仅显示首个active cue，候选和旧1.1.0/1.2.0
均受影响；1.0.0按索引合并后能显示双语。Firefox暂停偏移在三种核心上持续失败，
原生cue时间已经变为2至4秒，但播放时间1.5秒时仍列为active并显示。9项定向
复跑6通过3失败；WebKit5.3.0切源丢字幕首次失败后通过，保留待定位记录。
CI50项和测试/服务脚本lint通过；无生产代码、依赖、声明或产物改动。
见[检查点](changes/2026-09-14-PKG-MULTI-SUB-05-combinations.md)和
[证据](baselines/multiple-subtitles-combinations-validation.json)。247项中175 done、
17 doing、55 todo；两项新风险保持open，任务05不能完成。下一步先最小化并修复
暂停偏移缺陷，再解决旧核心双语与切源边界；ASS/编码/真机及分发仍未验收。

## PKG-MULTI-SUB-08 字幕实体与标签包装完成

通过解析器既有实体表入口修复标准命名实体后多余分号，保留原始vendor文件与
来源校验；独立转义字面文本，避免实体形式的标签/时间戳被再次解释。同步修复
把包装节点塞进b等标签注释字段的问题，改为节点外包装，保留CSS钩子与时间戳。
九套历史实现分别复现两类旧缺陷，最终11项新用例在旧bundle中8红3绿，候选及
实际main/legacy各11项全绿。完整插件294项、源码浏览器42项、实际main浏览器
6项、CI50项通过；严格TS与vendor校验通过，lint 1既有warning。三格式与docs
副本正常重建，公开类型/依赖/版本未改。详见[记录](changes/2026-09-14-PKG-MULTI-SUB-08-entities.md)
和[证据](baselines/multiple-subtitles-entities-validation.json)。关闭实体与标签包装
风险，247项变为175 done、16 doing、56 todo；这不完成全部旧核心/真机/分发
验收，下一步继续任务05组合验证。独立本地提交，无推送/发布。
