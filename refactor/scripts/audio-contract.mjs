...
export async function audioFunctionHash(code) {
  const source = ts.createSourceFile('audio.js', code, ts.ScriptTarget.Latest, true, ts.ScriptKind.JS)
  assert.equal(source.parseDiagnostics.length, 0, 'Invalid audio source')
  const functions = source.statements.filter(item => ts.isFunctionDeclaration(item) && item.name?.text === 'artplayerPluginAudioTrack')
  assert.equal(functions.length, 1, 'Expected exactly one audio factory')
  const normalized = await transform(functions[0].getText(source).replace(/^export\s+default\s+/, ''), {
    loader: 'js',
    target: 'es2020',
    minifySyntax: true,
...
export async function verifyAudioContract() {
  const baseline = JSON.parse(fs.readFileSync(path.join(refactorDir, 'baselines/audio-track-release.json'), 'utf8'))
  ...
  const sourceHash = await audioFunctionHash(historical.get(`packages/${release.name}/src/index.js`))
  const publishedHash = await audioFunctionHash(readMember(archive, `package/${manifest.module.replace(/^\.\//, '')}`).toString())
  assert.equal(sourceHash, baseline.functionComparison.sha256)
  assert.equal(publishedHash, sourceHash)
  console.log(`Verified ${release.name}@${release.version}: archive, ${Object.keys(release.files).length} members, historical sources and normalized factory`)
  return { baseline, archive }
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url))
  await verifyAudioContract()
