...
export async function verifyDpipContract() {
  const baseline = JSON.parse(fs.readFileSync(path.join(refactorDir, 'baselines/dpip-release.json'), 'utf8'))
  ...
  for (const release of [baseline.release, ...baseline.previous]) {
    ...
    assert.deepEqual(missing, release.missingEntrypoints, 'Missing runtime entries must remain explicit historical defects')
  ...
  for (const [file, expected] of Object.entries(baseline.source)) {
    ...
    assert.equal(hash(source), expected, `Document PiP frozen source: ${file}`)
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  console.log(`Document PiP verified: ${releases.length} archives, ${releases.reduce((total, release) => total + Object.keys(release.files).length, 0)} members; 1.0.0 missing runtime preserved explicitly`)
}
