...
2026-09-13 用户授权使用子代理加快重构。按独立包或不重叠文件划分有界任务，
主代理负责集成、共享任务台账和每任务提交；子代理不得同时改同一生产模块或
自行批量提交。测试可与契约核对并行，依赖未满足的实现不得提前宣布完成。
按实际影响范围验证，完整回归保留在跨包变更和阶段验收节点；不承诺线性加速。
...
2026-09-15 用户要求由其指导复盘：实施和必要验证完成后先交接，等待用户明确
启动 REVIEW-01/02/03，不自动进行首轮或后续全局复盘。各任务日常自查和回归
测试仍需完成；三轮复盘及发布门槛不删除、不标为已通过。以
[当前复盘启动规则](release-reviews.md)为准，减少重复验证和冗长进度输出。
