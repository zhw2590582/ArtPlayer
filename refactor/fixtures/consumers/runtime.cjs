...
;(async () => {
  const checks = []
  const observations = {}
  const check = (id, value) => { assert(value, id); checks.push(id) }
  ...
  check('DIST.cjs', typeof core === 'function' && core.version === '5.4.0' && typeof chapter({}) === 'function')
  ...
  for (const [name, filename, globalName] of [
    ...
    let value
    let calls = 0
    const define = (factory) => { calls++; value = factory() }
    define.amd = {}
  ...
  for (const Class of [core, esm.default, legacy.default]) {
    for (const useSSR of [false, true]) {
      let error
      try { new Class({ container: '#server', url: 'video.mp4', useSSR }) }
