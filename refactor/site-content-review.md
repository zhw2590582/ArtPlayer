...
2026-09-16收尾：SITE-04内容任务已完成。核心963条声明全部有唯一语义映射，
源码/指南哈希及HTML锚点已校验；20个生态包40份双语指南的既有结论与当前源码、
类型和维护文档一致。见[完成记录](changes/2026-09-16-SITE-04-complete.md)及
[证据](baselines/site04-display.json)。下面保留各批次进度作为历史记录；完整demo、
设备、SDK、发布和正式复盘不由此关闭。
...
页面检查见对应批次证据。当时核心963条仍待独立核对，随后按下面分组完成。
...
实例身份、生命周期与媒体能力随后新增61条映射，累计829条、134条待核对。
146项测试、10份严格TS、12组页面/42次转发及三浏览器各38项断言通过，见
[证据](baselines/site04-lifecycle.json)。env/build说明按冻结发布包与当前构建纠正，
118个既有片段及运行时不变。WebKit首次导航超时保留，专项重跑通过；可选媒体
能力只记录字段，不代替原生显示请求、真机或全量播放验证。

播放、数值、切源与画质随后新增71条映射，累计900条、63条待核对。85项回归、
24项源码范围真实媒体旧新对照、8份严格TS和6组页面/36次转发通过，见
[证据](baselines/site04-playback.json)。明确切源resolve包含取消等正常结束，
不等同于正在播放；94个既有示例及生产行为不变，安装包、真机、SDK门槛仍独立。

显示/尺寸/截图/预览最后新增63条映射，累计963条、0条待核对。122项回归、93项
source浏览器用例、10份严格TS和六组页面/36次转发通过；两个state示例修正为
先mini=true进入、再state='standard'退出。PiP支持、受控几何与真机缺口分别记录。

- 核心：963条成员均有对应指南、源码与语义结论，ID完全匹配当前inventory，
  不是仅按同名标题匹配；全部分组哈希及映射锚点在收尾时通过校验。
