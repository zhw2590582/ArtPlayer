...
2026-09-15 用户明确调整执行顺序：“当你完成了，先不要走复盘，让我来指导复盘”。
继续完成已授权的重构实施和必要测试；实施完成后先交接实际结果、剩余问题与证据，
等待用户明确指导和启动复盘。不得自动进入 REVIEW-01/02/03，也不提前启动首轮。
下文关于首轮可以提前进行的安排仅表示技术依赖允许，不构成当前启动授权。

三轮复盘及发布门槛保持原样，待用户指导后按其范围和顺序执行。等待期间不得
将复盘标为完成或声称发布就绪。日常实现自查、回归测试和已复现问题修复继续进行，
不能把必要验证省略为节省token，也不为消耗剩余额度反复运行已经充分验证的内容。
...
| Codex 内置浏览器 | Chrome 不可用时接续其支持的真实页面/API/媒体/交互验证 | 用户已明确授权自动回退；记录实际浏览器、版本、操作和限制，不将结果标成外部 Chrome 或真实设备通过 |
...
2026-09-10 用户补充：Chrome 不可用时使用内置浏览器。无需等待扩展恢复即可继续它支持的验证；若特定能力不支持，再将该能力登记为未验证并继续其他任务。内置浏览器已实际打开固定发布包的 api.html，9 项基础 API 检查通过，页面 errors 为空；播放推进、切源、设备/SDK 及完整 API 基线仍分别验收。
