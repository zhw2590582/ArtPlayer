...
Auto-thumbnail 的 04 现基于已完成的类型、入口及资源修复子任务独立验收；03 的首帧像素和剩余抽帧问题仍保持原验收要求，由 05 直接依赖 03 和 04。这样类型和版本准备可以继续，整包集成、分发及发布仍不能越过未完成的 03；计划校验器强制保留这条依赖。此调整不修改任何浏览器支持或像素断言。
...
chapter 作为早期试点，03/04 在已有核心上完成，不使用候选核心内部能力；随后 PILOT-01 先完成一次类型、浏览器、tarball 与维护文档闭环。05/06 仍在最终候选核心上重新验收，其他包的旧版本基线仍由各自 01 步负责。
