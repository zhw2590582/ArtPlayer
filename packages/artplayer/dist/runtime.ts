...
export interface Notice extends NoticeView { art: Artplayer }
...
interface Artplayer extends Player, Emitter<Events> {
  ...
  layers: Component
  controls: Controls
  contextmenu: Component & { init: () => void }
  subtitle: Subtitle
  info: Component & { init: () => void }
  loading: Component
...
export interface ArtplayerConstructor extends StaticOptions {
  ...
  readonly Emitter: new <Events extends { [Name in keyof Events]: readonly unknown[] } = Record<PropertyKey, unknown[]>>(...args: unknown[]) => Emitter<Events>
...
// eslint-disable-next-line ts/no-redeclare -- One existing constructor value and its instance view.
