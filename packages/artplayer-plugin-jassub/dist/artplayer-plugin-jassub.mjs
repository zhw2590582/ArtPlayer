/*!
 * artplayer-plugin-jassub.js v1.1.0
...
if (typeof HTMLVideoElement < "u" && !("requestVideoFrameCallback" in HTMLVideoElement.prototype) && "getVideoPlaybackQuality" in HTMLVideoElement.prototype) {
  HTMLVideoElement.prototype.requestVideoFrameCallback = function(callback) {
    ...
    const poll = (previous, now) => {
      ...
      const mediaFallback = initialFrames === 0 && frames === 0 && initial.totalVideoFrames === 0 && initial.droppedVideoFrames === 0 && quality.totalVideoFrames === 0 && quality.droppedVideoFrames === 0 && Number.isFinite(this.currentTime) && this.currentTime !== initialTime && this.readyState >= 2 && !this.seeking && now - started >= 1e3 / 30;
...
const _o = class _o extends EventTarget {
  ...
  constructor(e) {
    ...
    try {
      ...
      this._canvasctrl = this._offscreenRender ? this._canvas.transferControlToOffscreen() : this._canvas, this._ctx = !this._offscreenRender && this._canvasctrl.getContext("2d"), this._lastRenderTime = 0, this.debug = !!e.debug, this.prescaleFactor = e.prescaleFactor || 1, this.prescaleHeightLimit = e.prescaleHeightLimit || 1080, this.maxRenderHeight = e.maxRenderHeight || 0, this._boundResize = this.resize.bind(this), this._boundTimeUpdate = this._timeupdate.bind(this), this._boundSetRate = () => {
        if (!this._destroyed) this.setRate(this._video.playbackRate);
      }, this._boundUpdateColorSpace = this._updateColorSpace.bind(this), this._video && this.setVideo(e.video), this._onDemandRender && (this.busy = false, this._lastDemandTime = null), this._worker = new Worker(e.workerUrl || "jassub-worker.js"), this._worker.onmessage = (s) => this._onmessage(s), this._worker.onerror = (s) => this._error(s), t.then(() => {
  ...
  resize(e = 0, t = 0, s = 0, r = 0, n = this._video?.paused) {
    ...
    this._canvas.style.top = s + "px", this._canvas.style.left = r + "px", n && this.busy === false ? this.busy = true : n = false, this.sendMessage("canvas", { width: e, height: t, videoWidth: this._videoWidth || this._video?.videoWidth || e, videoHeight: this._videoHeight || this._video?.videoHeight || t, force: n });
  }
  ...
  */
  ...
  _handleRVFC(e, { mediaTime: t, width: s, height: r }) {
    if (this._destroyed)
      return null;
    this.busy ? this._lastDemandTime = { mediaTime: t, width: s, height: r } : (this.busy = true, this._demandRender({ mediaTime: t, width: s, height: r })), this._requestFrame();
  }
  _requestFrame() {
  ...
  _reAttachOffscreen() {
    if (this._destroyed || !this._offscreenRender || !this._ctx)
      return null;
    this._canvas.remove(), this._createCanvas(), this._canvasctrl = this._canvas.transferControlToOffscreen(), this._ctx = false, this.busy = false, this._lastDemandTime = null, this.sendMessage("offscreenCanvas", null, [this._canvasctrl]), this.resize(0, 0, 0, 0, true);
  }
  _updateColorSpace() {
  ...
  _render({ images: e, asyncRender: t, times: s, width: r, height: n, colorSpace: a }) {
    ...
    try {
      ...
      for (const i of e)
        i.image && (t ? this._ctx.drawImage(i.image, i.x, i.y) : (this._bufferCanvas.width = i.w, this._bufferCanvas.height = i.h, this._bufferCtx.putImageData(new ImageData(this._fixAlpha(new Uint8ClampedArray(i.image)), i.w, i.h), 0, 0), this._ctx.drawImage(this._bufferCanvas, i.x, i.y)));
    } finally {
      if (t) for (const i of e) i.image?.close();
    }
    if (this.debug) {
  ...
  _fetchFromWorker(e, t) {
    ...
    const onMessage = ({ data }) => {
      if (data.target === e.target) finish(null, data);
    };
