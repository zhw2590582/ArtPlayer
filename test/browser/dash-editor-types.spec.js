...
test('Monaco consumes DASH global types and executes the emitted plugin factory', async ({ page }, testInfo) => {
  const code = process.env.ARTPLAYER_DASH_ARTIFACT
    ? fs.readFileSync(process.env.ARTPLAYER_DASH_ARTIFACT, 'utf8')
    : await compilePackage('artplayer-plugin-dash-control', 'umd')
  const declarations = Object.fromEntries(['artplayer', 'artplayer-plugin-dash-control'].map(name => [name, hash(fs.readFileSync(new URL(`../../docs/assets/ts/${name}.d.ts`, import.meta.url)))]))
  const inputs = { candidate: process.env.ARTPLAYER_DASH_ARTIFACT || 'workspace source build', sha256: hash(code), declarations }
  await testInfo.attach('dash-editor-inputs', { contentType: 'application/json', body: JSON.stringify(inputs) })
  ...
  const result = await page.evaluate(async () => {
    ...
    const names = ['artplayer', 'artplayer-plugin-dash-control']
...
const dashOption: artplayerPluginDashControl.Option = {
  quality: { getName: level => String(level.height) },
...
`, 'typescript', api.Uri.parse('file:///dash-consumer.ts'))
    ...
    try {
      ...
      if (!script)
        throw new Error('Monaco did not emit the DASH fixture')
