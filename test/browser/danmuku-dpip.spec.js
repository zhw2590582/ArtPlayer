...
test.beforeAll(async () => {
  danmuku = process.env.ARTPLAYER_DANMUKU_ARTIFACT
...
for (const core of ['published', 'candidate']) {
  for (const ending of ['close', 'native-close', 'destroy']) {
    test(`${core}: Danmuku, heatmap and Worker survive native Document PiP ${ending}`, async ({ page, context }, testInfo) => {
      await page.goto(`/test/player.html?core=${core}`)
      const supported = await page.evaluate(() => typeof window.documentPictureInPicture?.requestWindow === 'function')
      const evidence = { core, ending, supported, danmuku: { input: process.env.ARTPLAYER_DANMUKU_ARTIFACT || 'source UMD', sha256: hash(danmuku) }, dpip: { input: dpip.name, sha256: hash(dpip.code) }, outcome: 'incomplete', rounds: [] }
      if (!supported) {
