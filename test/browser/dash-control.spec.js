...
async function openDash(page, core, plugin, testInfo) {
  ...
  for (const plugin of ['published', 'candidate']) {
    for (const surface of ['control', 'setting']) {
      for (const type of ['quality', 'audio']) {
        test(`${core} core + ${plugin} DASH: controlled SDK ${type} selection through real ${surface} DOM`, async ({ page }, testInfo) => {
          await openDash(page, core, plugin, testInfo)
          ...
          const expected = type === 'quality'
            ? [['auto', false], plugin === 'published' ? ['qualityIndex', 'video', 1] : ['representationId', 'video', 'high']]
            : [['track', 'fr', true]]
