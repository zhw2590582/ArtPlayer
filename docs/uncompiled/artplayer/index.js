            ...
            <div class="art-info">
              <div class="art-info-panel">
                <div class="art-info-item">
                  ...
                  <div class="art-info-content">${"4.7.0"}</div>
                </div>
                <div class="art-info-item">
...
function miniMix(art) {
    const { storage , proxy , template: { $player , $video  }  } = art;
    let isDroging = false;
    let lastPageX = 0;
    let lastPageY = 0;
    function hideMini() {
        const { $mini  } = art.template;
        if ($mini) return (0, _utils.setStyle)($mini, "display", "none");
    }
    function createMini() {
        const { $mini  } = art.template;
        if ($mini) {
...
function version(option) {
    return {
        ...option,
        html: `<a href="https://artplayer.org" target="_blank">ArtPlayer ${"4.7.0"}</a>`
    };
}
...
function autoPlayback(art) {
    ...
    if (art.option.backdrop) (0, _utils.addClass)($autoPlayback, "art-backdrop-filter");
    art.on("video:timeupdate", ()=>{
