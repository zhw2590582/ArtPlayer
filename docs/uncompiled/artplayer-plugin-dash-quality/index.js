...
})({"gigZd":[function(require,module,exports) {
...
function artplayerPluginDashQuality(option) {
    return (art)=>{
        const { $video  } = art.template;
        const { errorHandle  } = art.constructor.utils;
        function update() {
            const dash = art.dash || window.dash;
            errorHandle(dash && dash.getVideoElement() === $video, 'Cannot find instance of Dash from "art.dash" or "window.dash"');
