...
})({"d19bT":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _tinyEmitter = require("tiny-emitter");
var _tinyEmitterDefault = parcelHelpers.interopDefault(_tinyEmitter);
var _utils = require("./utils");
class ArtplayerToolThumbnail extends _tinyEmitterDefault.default {
    constructor(option = {}){
        super();
    ...
    setup(option = {}) {
        ...
        this.option.number = _utils.clamp(number, 10, 1000);
        this.option.width = _utils.clamp(width, 10, 1000);
        this.option.column = _utils.clamp(column, 1, 1000);
        return this;
    }
    static creatVideo() {
    ...
    start() {
        if (!this.video.duration) return _utils.sleep(1000).then(()=>this.start()
        );
        const { width , number , begin , end  } = this.option;
        const height = this.video.videoHeight / this.video.videoWidth * width;
        this.option.height = height;
        this.option.begin = _utils.clamp(begin, 0, this.video.duration);
        this.option.end = _utils.clamp(end || this.video.duration, begin, this.video.duration);
        ...
        return _utils.runPromisesInSeries(promiseList).then(()=>{
            this.processing = false;
            this.emit('done');
        }).catch((err)=>{
            this.processing = false;
    ...
    download() {
        ...
        const name = `${_utils.getFileName(this.file.name)}.png`;
        elink.download = name;
...
},{"tiny-emitter":"ltU3o","./utils":"OYkYn","@parcel/transformer-js/src/esmodule-helpers.js":"5dUr6"}],"ltU3o":[function(require,module,exports) {
function E() {
...
},{}],"OYkYn":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
function runPromisesInSeries(ps) {
    return ps.reduce((p, next)=>p.then(next)
    , Promise.resolve());
}
