...
artplayerPluginDanmuku.build = "2023-05-01 20:37:59";
...
function setting(art, danmuku) {
    ...
    const { template: { $controlsCenter , $player  } , constructor: { utils: { removeClass , addClass , append , setStyle , tooltip , query , inverseClass , getIcon  }  }  } = art;
...
function heatmap(art, danmuku) {
    const { constructor: { utils: { query , setStyles  }  } , template: { $progress  }  } = art;
    const height = 30;
        ...
        mounted ($heatmap) {
            ...
            function update() {
                ...
                for(let index = 1; index <= $canvas.width; index += 1){
                    const length = danmuku.danmus.filter((item)=>item.time > (index - 1) * gap && item.time <= index * gap).length + 5;
                    points.push({
                        x: index - 1,
                        y: height - length
                    });
