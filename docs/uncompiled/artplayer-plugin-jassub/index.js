...
})({"5fyRf":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "default", ()=>artplayerPluginJassub);
var _jassubEsJs = require("./jassub.es.js");
var _jassubEsJsDefault = parcelHelpers.interopDefault(_jassubEsJs);
function artplayerPluginJassub(option) {
    return (art)=>{
        const instance = new (0, _jassubEsJsDefault.default)({
            video: art.video,
            ...option
        });
...
if (typeof window !== 'undefined') window.artplayerPluginJassub = artplayerPluginJassub;

},{"./jassub.es.js":"4L4Zz","@parcel/transformer-js/src/esmodule-helpers.js":"8oCsH"}],"4L4Zz":[function(require,module,exports,__globalThis) {
...
typeof HTMLVideoElement < "u" && !("requestVideoFrameCallback" in HTMLVideoElement.prototype) && "getVideoPlaybackQuality" in HTMLVideoElement.prototype && (HTMLVideoElement.prototype._rvfcpolyfillmap = {}, HTMLVideoElement.prototype.requestVideoFrameCallback = function(c) {
    const e = performance.now(), t = this.getVideoPlaybackQuality(), s = this.mozPresentedFrames || this.mozPaintedFrames || t.totalVideoFrames - t.droppedVideoFrames, r = (n, a)=>{
        ...
        } else this._rvfcpolyfillmap[e] = requestAnimationFrame((d)=>r(a, d));
    };
...
class o extends EventTarget {
   ...
   */ constructor(e){
        ...
        this._loaded = /** @type {Promise<void>} */ new Promise((s)=>{
            this._init = s;
        });
        const t = o._test();
        if (this._onDemandRender = "requestVideoFrameCallback" in HTMLVideoElement.prototype && (e.onDemandRender ?? !0), this._offscreenRender = "transferControlToOffscreen" in HTMLCanvasElement.prototype && !e.canvas && (e.offscreenRender ?? !0), this.timeOffset = e.timeOffset || 0, this._video = e.video, this._videoHeight = 0, this._videoWidth = 0, this._videoColorSpace = null, this._canvas = e.canvas, this._video && !this._canvas) this._canvasParent = document.createElement("div"), this._canvasParent.className = "JASSUB", this._canvasParent.style.position = "relative", this._canvas = this._createCanvas(), this._video.insertAdjacentElement("afterend", this._canvasParent);
        else if (!this._canvas) throw this.destroy("Don't know where to render: you should give video or canvas in options.");
        ...
        this._canvasctrl = this._offscreenRender ? this._canvas.transferControlToOffscreen() : this._canvas, this._ctx = !this._offscreenRender && this._canvasctrl.getContext("2d"), this._lastRenderTime = 0, this.debug = !!e.debug, this.prescaleFactor = e.prescaleFactor || 1, this.prescaleHeightLimit = e.prescaleHeightLimit || 1080, this.maxRenderHeight = e.maxRenderHeight || 0, this._boundResize = this.resize.bind(this), this._boundTimeUpdate = this._timeupdate.bind(this), this._boundSetRate = this.setRate.bind(this), this._boundUpdateColorSpace = this._updateColorSpace.bind(this), this._video && this.setVideo(e.video), this._onDemandRender && (this.busy = !1, this._lastDemandTime = null), this._worker = new Worker(e.workerUrl || "jassub-worker.js"), this._worker.onmessage = (s)=>this._onmessage(s), this._worker.onerror = (s)=>this._error(s), t.then(()=>{
            ...
            }), this._offscreenRender === !0 && this.sendMessage("offscreenCanvas", null, [
                this._canvasctrl
            ]);
        });
    }
    ...
    /** @type {boolean|null} */ static _supportsSIMD = null;
    /** @type {boolean|null} */ static _hasAlphaBug = null;
    /** @type {boolean|null} */ static _hasBitmapBug = null;
    static _testSIMD() {
        if (o._supportsSIMD === null) try {
    ...
    static async _testImageBugs() {
        ...
        if (!r) throw new Error("Canvas rendering not supported");
        e.width = s.width = 1, e.height = s.height = 1, t.clearRect(0, 0, 1, 1), r.clearRect(0, 0, 1, 1);
        const n = r.getImageData(0, 0, 1, 1).data;
        t.putImageData(new ImageData(new Uint8ClampedArray([
            0,
            255,
            0,
            0
        ]), 1, 1), 0, 0), r.drawImage(e, 0, 0);
   ...
   */ resize(e = 0, t = 0, s = 0, r = 0, n = this._video?.paused) {
        if ((!e || !t) && this._video) {
            ...
            } else i = this._computeCanvasSize(a.width || 0, a.height || 0);
        ...
        this._canvas.style.top = s + "px", this._canvas.style.left = r + "px", n && this.busy === !1 ? this.busy = !0 : n = !1, this.sendMessage("canvas", {
            width: e,
            height: t,
   ...
   */ setTrackByUrl(e) {
        this.sendMessage("setTrackByUrl", {
            url: e
        }), this._reAttachOffscreen(), this._ctx && (this._ctx.filter = "none");
    }
    /**
   * Overwrites the current subtitle content.
   * @param  {String} content Content of the ASS file.
   */ setTrack(e) {
        this.sendMessage("setTrack", {
            content: e
        }), this._reAttachOffscreen(), this._ctx && (this._ctx.filter = "none");
    }
    /**
  ...
  */ /**
   ...
   */ styleOverride(e) {
        this.sendMessage("styleOverride", {
            style: e
        });
    }
    /**
  ...
  */ /**
   ...
   */ setDefaultFont(e) {
    ...
    _handleRVFC(e, { mediaTime: t, width: s, height: r }) {
        ...
        this.busy ? this._lastDemandTime = {
            mediaTime: t,
            width: s,
            height: r
        } : (this.busy = !0, this._demandRender({
            mediaTime: t,
            width: s,
            height: r
    ...
    _demandRender({ mediaTime: e, width: t, height: s }) {
        this._lastDemandTime = null, (t !== this._videoWidth || s !== this._videoHeight) && (this._videoWidth = t, this._videoHeight = s, this.resize()), this.sendMessage("demand", {
            time: e + this.timeOffset
        });
    }
    ...
    _reAttachOffscreen() {
        if (!this._offscreenRender || !this._ctx) return null;
        this._canvas.remove(), this._createCanvas(), this._canvasctrl = this._canvas.transferControlToOffscreen(), this._ctx = !1, this.sendMessage("offscreenCanvas", null, [
            this._canvasctrl
        ]), this.resize(0, 0, 0, 0, !0);
    }
    _updateColorSpace() {
   ...
   */ _verifyColorSpace({ subtitleColorSpace: e, videoColorSpace: t = this._videoColorSpace }) {
    ...
    _render({ images: e, asyncRender: t, times: s, width: r, height: n, colorSpace: a }) {
        this._unbusy(), this.debug && (s.IPCTime = Date.now() - s.JSRenderTime), (this._canvasctrl.width !== r || this._canvasctrl.height !== n) && (this._canvasctrl.width = r, this._canvasctrl.height = n, this._verifyColorSpace({
            subtitleColorSpace: a
        })), this._ctx.clearRect(0, 0, this._canvasctrl.width, this._canvasctrl.height);
        for (const i of e)i.image && (t ? (this._ctx.drawImage(i.image, i.x, i.y), i.image.close()) : (this._bufferCanvas.width = i.w, this._bufferCanvas.height = i.h, this._bufferCtx.putImageData(new ImageData(this._fixAlpha(new Uint8ClampedArray(i.image)), i.w, i.h), 0, 0), this._ctx.drawImage(this._bufferCanvas, i.x, i.y)));
        if (this.debug) {
            s.JSRenderTime = Date.now() - s.JSRenderTime - s.IPCTime;
            let i = 0;
            const h = s.bitmaps || e.length;
            delete s.bitmaps;
            for(const d in s)i += s[d];
            console.log("Bitmaps: " + h + " Total: " + (i | 0) + "ms", s);
        }
    }
    _fixAlpha(e) {
        if (o._hasAlphaBug) for(let t = 3; t < e.length; t += 4)e[t] = e[t] > 1 ? e[t] : 1;
        return e;
    }
    _ready() {
   ...
   */ async sendMessage(e, t = {}, s) {
    ...
    _error(e) {
        const t = e instanceof Error ? e : e instanceof ErrorEvent ? e.error : new Error(e), s = e instanceof Event ? new ErrorEvent(e.type, e) : new ErrorEvent("error", {
            error: t
        });
